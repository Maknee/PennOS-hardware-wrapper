...
#define SET_EXPR_LOCATION(NODE, LOCUS) EXPR_CHECK((NODE)) \
                                           ->exp.locus    \
    = (LOCUS)
...
#define SSA_NAME_DEF_STMT(NODE) SSA_NAME_CHECK(NODE) \
                                    ->ssa_name.def_stmt

/* Returns the SSA version number of this SSA name.  Note that in
   tree SSA, version numbers are not per variable and may be recycled.  */
#define SSA_NAME_VERSION(NODE) SSA_NAME_CHECK(NODE) \
                                   ->base.u.version
...
#define SSA_NAME_IMM_USE_NODE(NODE) SSA_NAME_CHECK(NODE) \
                                        ->ssa_name.imm_uses
...
#if (GCC_VERSION >= 2007)
#define DECL_RTL_KNOWN_SET(decl) __extension__({  tree const __d = (decl);						\
...
#define DECL_POSSIBLY_INLINED(DECL) \
    FUNCTION_DECL_CHECK(DECL)       \
        ->function_decl.possibly_inlined
