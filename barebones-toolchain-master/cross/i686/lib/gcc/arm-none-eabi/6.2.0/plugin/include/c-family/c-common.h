...
#define OBJC_IS_PATTR_KEYWORD(rid) \
    ((unsigned int)(rid) >= (unsigned int)RID_FIRST_PATTR && (unsigned int)(rid) <= (unsigned int)RID_LAST_PATTR)
...
inline omp_clause_mask::omp_clause_mask()
{
}

inline omp_clause_mask::omp_clause_mask(uint64_t l)
    : low(l)
    , high(0)
{
}

inline omp_clause_mask::omp_clause_mask(uint64_t l, uint64_t h)
    : low(l)
    , high(h)
{
}
...
extern void cilkplus_extract_an_triplets(vec<tree, va_gc>*, size_t, size_t,
    vec<vec<an_parts> >*);
extern vec<tree, va_gc>* fix_sec_implicit_args(location_t, vec<tree, va_gc>*, vec<an_loop_parts>, size_t, tree);
