...
#define BINARY_PREDICATE(OP, F) \
    template <typename T>       \
    bool OP(const T& c) const   \
    {                           \
        return wi::F(*this, c); \
    }

#define UNARY_OPERATOR(OP, F)         \
    WI_UNARY_RESULT(generic_wide_int) \
    OP()                              \
    const                             \
    {                                 \
        return wi::F(*this);          \
    ...
    wi::div_round(const T1& x, const T2& y, signop sgn, bool* overflow)
{
    ...
    if (remainder != 0)
    {
        ...
        {
            WI_BINARY_RESULT(T1, T2)
            abs_remainder = wi::abs(remainder);
...
wi::lrotate(const T1& x, const T2& y, unsigned int width)
{
    ...
    if (width == 0)
        width = precision;
    WI_UNARY_RESULT(T2)
    ymod = umod_trunc(y, width);
    WI_UNARY_RESULT(T1)
    left = wi::lshift(x, ymod);
    WI_UNARY_RESULT(T1)
    right = wi::lrshift(x, wi::sub(width, ymod));
    if (width != precision)
...
wi::rrotate(const T1& x, const T2& y, unsigned int width)
{
    ...
    ymod = umod_trunc(y, width);
    WI_UNARY_RESULT(T1)
    right = wi::lrshift(x, ymod);
    WI_UNARY_RESULT(T1)
    left = wi::lshift(x, wi::sub(width, ymod));
    if (width != precision)
        return wi::zext(left, width) | wi::zext(right, width);
    return left | right;
}
