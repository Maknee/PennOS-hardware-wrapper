...
#define IRA_BB_NODE_BY_INDEX(index) __extension__(({ ira_loop_tree_node_t _node = (&ira_bb_nodes[index]);		\
...
#if defined ENABLE_IRA_CHECKING && (GCC_VERSION >= 2007)
#define IRA_LOOP_NODE_BY_INDEX(index) __extension__(({ ira_loop_tree_node_t const _node = (&ira_loop_nodes[index]);	\
...
#define SET_MINMAX_SET_BIT(R, I, MIN, MAX) __extension__(({ int _min = (MIN), _max = (MAX), _i = (I);				\
     if (_i < _min || _i > _max)					\
       {								\
         fprintf (stderr,						\
...
#define CLEAR_MINMAX_SET_BIT(R, I, MIN, MAX) __extension__(({ int _min = (MIN), _max = (MAX), _i = (I);				\
     if (_i < _min || _i > _max)					\
       {								\
         fprintf (stderr,						\
                  "\n%s: %d: error in %s: %d not in range [%d,%d]\n",   \
...
#define TEST_MINMAX_SET_BIT(R, I, MIN, MAX) __extension__(({ int _min = (MIN), _max = (MAX), _i = (I);				\
     if (_i < _min || _i > _max)					\
       {								\
         fprintf (stderr,						\
                  "\n%s: %d: error in %s: %d not in range [%d,%d]\n",   \
