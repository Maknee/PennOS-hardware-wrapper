...
#define THUNK_FUNCTION_CHECK(NODE) __extension__({  __typeof (NODE) const __t = (NODE);					\
...
#define LANG_TYPE_CLASS_CHECK(NODE) __extension__({  struct lang_type *lt = TYPE_LANG_SPECIFIC (NODE);		\
    if (! lt->u.h.is_lang_type_class)				\
      lang_check_failed (__FILE__, __LINE__, __FUNCTION__);	\
    &lt->u.c; })

#define LANG_TYPE_PTRMEM_CHECK(NODE) __extension__({  struct lang_type *lt = TYPE_LANG_SPECIFIC (NODE);		\
    if (lt->u.h.is_lang_type_class)				\
      lang_check_failed (__FILE__, __LINE__, __FUNCTION__);	\
    &lt->u.ptrmem; })
...
#define LANG_DECL_MIN_CHECK(NODE) __extension__({ struct lang_decl *lt = DECL_LANG_SPECIFIC (NODE);		\
...
#define LANG_DECL_FN_CHECK(NODE) __extension__({ struct lang_decl *lt = DECL_LANG_SPECIFIC (STRIP_TEMPLATE (NODE));	\
   if (!DECL_DECLARES_FUNCTION_P (NODE) || lt->u.base.selector != 1)	\
     lang_check_failed (__FILE__, __LINE__, __FUNCTION__);		\
   &lt->u.fn; })

#define LANG_DECL_NS_CHECK(NODE) __extension__({ struct lang_decl *lt = DECL_LANG_SPECIFIC (NODE);			\
   if (TREE_CODE (NODE) != NAMESPACE_DECL || lt->u.base.selector != 2)	\
     lang_check_failed (__FILE__, __LINE__, __FUNCTION__);		\
   &lt->u.ns; })

#define LANG_DECL_PARM_CHECK(NODE) __extension__({ struct lang_decl *lt = DECL_LANG_SPECIFIC (NODE);		\
  if (TREE_CODE (NODE) != PARM_DECL)				\
    lang_check_failed (__FILE__, __LINE__, __FUNCTION__);	\
  &lt->u.parm; })

#define LANG_DECL_U2_CHECK(NODE, TF) __extension__({  struct lang_decl *lt = DECL_LANG_SPECIFIC (NODE);		\
...
#define CLASSTYPE_PRIMARY_TEMPLATE_TYPE(TYPE)                                                       \
    ((CLASSTYPE_USE_TEMPLATE((TYPE))                                                                \
         && !CLASSTYPE_TEMPLATE_SPECIALIZATION((TYPE)))                                             \
            ? TREE_TYPE(DECL_TEMPLATE_RESULT(DECL_PRIMARY_TEMPLATE(CLASSTYPE_TI_TEMPLATE((TYPE))))) \
            : (TYPE))
...
#define TYPE_SET_PTRMEMFUNC_TYPE(NODE, VALUE)                                                     \
    ...
    {                                                                                             \
        ...
        {                                                                                         \
            TYPE_LANG_SPECIFIC(NODE)                                                              \
                = (struct lang_type*)ggc_internal_cleared_alloc(sizeof(struct lang_type_ptrmem)); \
            TYPE_LANG_SPECIFIC(NODE)                                                              \
                ->u.ptrmem.h.is_lang_type_class                                                   \
                = 0;                                                                              \
        }                                                                                         \
        TYPE_LANG_SPECIFIC(NODE)                                                                  \
            ->u.ptrmem.record                                                                     \
...
struct cp_declarator
{
    ...
    ENUM_BITFIELD(cp_declarator_kind)
    kind : 4;
