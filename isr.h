...
// ! typedef for specific handler functions
typedef void (*isr_handler)(interrupt_registers* registers);

// ! set a handler to some isr/irq
void register_interrupt_handler(uint8_t index, isr_handler handler);

// ! init isr and irqs
void isr_init();

// ! wraps the code into c code that calls the index handler for irq
uint32_t irq_wrapper_handler(interrupt_registers* registers);

// ! wraps the code into c code that calls the index handler
